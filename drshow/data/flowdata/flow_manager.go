...
func ValidateDrpPlayerPipedIn() (*bufio.Reader, bool) {
	...
	case <-time.After(time.Duration(CFG.WaitTimeForSamplesInSec) * time.Second):
		log.Printf("No input recieved from stdin after %d seconds. This can be increased in the config file (%s).", CFG.WaitTimeForSamplesInSec, assets.RELEASE_CFG_PATH+assets.CFG_FILE_NAME)
