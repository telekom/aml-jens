...
import (
	...
	drpmultiplay "github.com/telekom/aml-jens/pkg/drp_multiplayer"
)
...
func ArgParse() (err error) {
	...
	multisession := config.PlayCfg().A_MultiSession
...
func exithandler(player *drpmultiplay.DrpMultiPlayer, exit chan uint8) {
...
func main() {
	logging.InitLogger(assets.NAME_DRMULTIPLAY)
	INFO.Printf("===>Starting DrMultiPlay @%s <===\n\n", time.Now().String())
	...
	cfg := config.PlayCfg()
	player := drpmultiplay.NewMultiPlayer(cfg)
