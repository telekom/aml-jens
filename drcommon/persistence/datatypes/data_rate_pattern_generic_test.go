...
const GOOD_PATH = assets.TESTDATA_DPR_DIR + DRP3VALL
const SAW_PATH = assets.TESTDATA_DPR_DIR + "saw.csv"
...
func TestBroken(t *testing.T) {
	files, err := ioutil.ReadDir(assets.TESTDATA_DPR_DIR)
	if err != nil {
	...
	for _, v := range files {
		...
		path := filepath.Join(assets.TESTDATA_DPR_DIR, v.Name())
		drp := datatypes.DB_data_rate_pattern{
			Scale:        1,
			MinRateKbits: 20400,
			Loop:         false,
