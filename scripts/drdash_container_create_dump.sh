...
#under the License.
#
# This script is meant to be run on the machine / vm with
# docker installed OR be able to ssh into that machine.
#
# Case A) Directly on the machine:
#       change `ssh` variable to `""`
# Case B) Via ssh
#       modify variable `ssh` or `ssh_target` according
#        to your needs
ssh_target="jens"
ssh="ssh $ssh_target "
scp="scp $ssh_target:"
